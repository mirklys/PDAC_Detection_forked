...
def get_args_parser():
    ...
    parser.add_argument(
        ...
        "--model_dir",
        type=str,
        default="./workspace/nnUNet_results",
        help="model directory for nnU-Net",
    )
    parser.add_argument(
        "--inv_alpha",
        type=int,
        default=15,
        help="parameter to control the expansion of the lesion",
    )
    ...
    with tqdm(total=len(img_paths)) as pbar:
        for img_path in img_paths:
            ...
            image_resampled = resample_img(
                itk_img, resample, is_label=False, out_size=[]
            )
            sitk.WriteImage(
                image_resampled,
                osp.join(
                    img_save_dir, osp.basename(img_path).replace(ext, "_0000.nii.gz")
                ),
            )
            pbar.update(1)
...
def crop_roi(img_dir, low_msk_dir, save_img_dir, margins=[100, 50, 15]):
    ...
    with tqdm(total=len(img_paths)) as pbar:
        for img_path in img_paths:
            ext = get_file_extension(img_path)
            low_msk_path = osp.join(
                low_msk_dir, osp.basename(img_path).replace(ext, ".nii.gz")
            )
            ...
            start_point_physical = low_msk.TransformIndexToPhysicalPoint(
                start_point_coordinates
            )
            finish_point_physical = low_msk.TransformIndexToPhysicalPoint(
            ...
            sitk.WriteImage(
                cropped_image,
                osp.join(
                    save_img_dir, osp.basename(img_path).replace(ext, "_0000.nii.gz")
                ),
            )
            pbar.update(1)
...
def GetFullSizDetectionMap(prediction_np, crop_coordinates, full_image, inv_alpha=15):
    lesion_candidates, confidences, indexed_pred = extract_lesion_candidates(
        prediction_np, dynamic_threshold_factor=inv_alpha
    )
...
def run(args):
    ...
    clinical_info_path = osp.join(
        args.input_dir, "clinical-information-pancreatic-ct.json"
    )
    ...
    for npz_fp, img_fp in zip(npz_fps, img_fps):
        ...
        assert (
            osp.basename(img_fp)[: -len(ext)] == filename
        ), f"{osp.basename(img_fp)[:-len(ext)]} and {filename}"
        ...
        detection_map, patient_level_prediction = GetFullSizDetectionMap(
            ...
            crop_coordinates[filename],
            itk_img,
            args.inv_alpha,
        )
        detection_map_fp = osp.join(
            args.output_dir, "pdac-detection-map", f"{filename}.nii.gz"
        )
    ...
    write_json_file(
        location=osp.join(args.output_dir, f"pdac-likelihood.json"), content=likelohood
    )
    print(f"\nInference time: {(time.time()-start)/len(npz_fps):.2f} seconds per image")
    print(
        f"Patient-level scores  are saved at: {osp.join(args.output_dir, f'pdac-likelihood.json')}"
    )
    print(
        f"Lesion detection maps are saved at: {osp.join(args.output_dir, 'pdac-detection-map')}\n"
    )
...
def print_info():
    ...
    print(
        "Welcome to use Team DTI's PDAC detection models (1st place in the PANORAMA challenge)\n"
    )
    ...
    print(
        "If you have questions or suggestions, feel free to open an issue at https://github.com/han-liu/PDAC_Detection/\n"
    )
