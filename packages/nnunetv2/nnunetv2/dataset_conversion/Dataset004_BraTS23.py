    ...
    with tqdm(total=len(case_ids)) as pbar:
        for i, c in enumerate(case_ids):
            shutil.copy(
                ...
                join(imagestr, f"Brats2023_{i+1:03d}" + "_0000.nii.gz"),
            )
            shutil.copy(
                join(brats_data_dir, c, c + "-t1c.nii.gz"),
                join(imagestr, f"Brats2023_{i+1:03d}" + "_0001.nii.gz"),
            )
            shutil.copy(
                join(brats_data_dir, c, c + "-t2w.nii.gz"),
                join(imagestr, f"Brats2023_{i+1:03d}" + "_0002.nii.gz"),
            )
            shutil.copy(
                join(brats_data_dir, c, c + "-t2f.nii.gz"),
                join(imagestr, f"Brats2023_{i+1:03d}" + "_0003.nii.gz"),
            )
            copy_BraTS_segmentation_and_convert_labels_to_nnUNet(
                join(brats_data_dir, c, c + "-seg.nii.gz"),
                join(labelstr, f"Brats2023_{i+1:03d}" + ".nii.gz"),
            )
            pbar.update(1)
