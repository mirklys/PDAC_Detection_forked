...
def convert_msd_dataset(
    ...
    assert len(existing_datasets) == 0, (
        f"Target dataset id {target_id} is already taken, please consider changing "
    ...
    with multiprocessing.get_context("spawn").Pool(num_processes) as p:
        ...
        source_images = [
            i
            for i in subfiles(imagesTr, suffix=".nii.gz", join=False)
        ...
        results.append(
            p.starmap_async(
                split_4d_nifti,
                zip(source_images, [target_imagesTr] * len(source_images)),
            )
        )
        ...
        source_images = [
            i
            for i in subfiles(imagesTs, suffix=".nii.gz", join=False)
            if not i.startswith(".") and not i.startswith("_")
        ]
        ...
        results.append(
            p.starmap_async(
                split_4d_nifti,
                zip(source_images, [target_imagesTs] * len(source_images)),
            )
        )
        ...
        source_images = [
            i
            for i in subfiles(labelsTr, suffix=".nii.gz", join=False)
            if not i.startswith(".") and not i.startswith("_")
        ]
    ...
    save_json(
        dataset_json,
        join(nnUNet_raw, target_dataset_name, "dataset.json"),
        sort_keys=False,
    )
...
if __name__ == "__main__":
    convert_msd_dataset(
        "/home/fabian/Downloads/Task05_Prostate", overwrite_target_id=201
    )
