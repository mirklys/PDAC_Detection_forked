...
def generate_overlay(
    ...
    for l in mapping.keys():
        image[segmentation == l] += overlay_intensity * np.array(
            hex_to_rgb(color_cycle[mapping[l]])
        )
...
def plot_overlay(
    ...
    assert all(
        [i == j for i, j in zip(image.shape, seg.shape)]
    ), "image and seg do not have the same shape: %s, %s" % (
        image_file,
        segmentation_file,
    )
    ...
    overlay = generate_overlay(
        image[selected_slice], seg[selected_slice], overlay_intensity=overlay_intensity
    )
...
def generate_overlays_from_raw(
    ...
    identifiers = get_identifiers_from_splitted_dataset_folder(
        join(folder, "imagesTr"), dataset_json["file_ending"]
    )

    image_files = [
        join(folder, "imagesTr", i + "_%04.0d.nii.gz" % channel_idx)
