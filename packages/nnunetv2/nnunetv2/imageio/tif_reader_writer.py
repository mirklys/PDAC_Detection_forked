...
class Tiff3DIO(BaseReaderWriter):
    ...
    def read_images(
        ...
        assert (
            ending.lower() in self.supported_file_endings
        ...
        if isfile(expected_aux_file):
            ...
            assert len(spacing) == 3, (
                "spacing must have 3 entries, one for each dimension of the image. File: %s"
                % expected_aux_file
            )
        else:
            print(
                f"WARNING no spacing file found for images {image_fnames}\nAssuming spacing (1, 1, 1)."
            )
            spacing = (1, 1, 1)
    ...
    def write_seg(self, seg: np.ndarray, output_fname: str, properties: dict) -> None:
        ...
        save_json(
            ...
            join(out_dir, file[: -(len(ending) + 1)] + ".json"),
        )
    ...
    def read_seg(self, seg_fname: str) -> Tuple[np.ndarray, dict]:
        ...
        if isfile(expected_aux_file):
            ...
            assert all(
                [i > 0 for i in spacing]
            ), f"Spacing must be > 0, spacing: {spacing}"
        else:
            print(
                f"WARNING no spacing file found for segmentation {seg_fname}\nAssuming spacing (1, 1, 1)."
            )
            spacing = (1, 1, 1)
