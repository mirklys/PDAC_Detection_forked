...
def export_pretrained_model(
    ...
    with zipfile.ZipFile(output_file, "w", zipfile.ZIP_DEFLATED) as zipf:
        for c in configurations:
            ...
            expected_fold_folder = [
                "fold_%s" % i if i != "all" else "fold_all" for i in folds
            ]
            assert all(
                [isdir(join(trainer_output_dir, i)) for i in expected_fold_folder]
            ), f"not all requested folds are present; {dataset_name} {c}; requested folds: {folds}"

            assert isfile(
                join(trainer_output_dir, "plans.json")
            ), f"plans.json missing, {dataset_name} {c}"
            ...
            for fold_folder in expected_fold_folder:
                ...
                if isfile(source_file):
                    zipf.write(
                        source_file, os.path.relpath(source_file, nnUNet_results)
                    )
                ...
                source_file = join(
                    trainer_output_dir, fold_folder, "network_architecture.pdf"
                )
                if isfile(source_file):
                    zipf.write(
                        source_file, os.path.relpath(source_file, nnUNet_results)
                ...
                if export_crossval_predictions:
                    ...
                    files = [
                        i
                        for i in subfiles(source_folder, join=False)
                        if not i.endswith(".npz") and not i.endswith(".pkl")
                    ]
                    for f in files:
                        zipf.write(
                            join(source_folder, f),
                            os.path.relpath(join(source_folder, f), nnUNet_results),
                        )
                # just the summary.json file from the validation
                else:
                    source_file = join(
                        trainer_output_dir, fold_folder, "validation", "summary.json"
                    )
                    zipf.write(
                        source_file, os.path.relpath(source_file, nnUNet_results)
                    )
            ...
            source_folder = join(
                trainer_output_dir,
                f"crossval_results_folds_{folds_tuple_to_string(folds)}",
            )
        ...
        for ens in subd:
            identifiers, folds = convert_ensemble_folder_to_model_identifiers_and_folds(
                ens
            )
            ok = True
            ...
            if ok:
                ...
                for s in source_files:
                    zipf.write(s, os.path.relpath(s, nnUNet_results))
        inference_information_file = join(
            nnUNet_results, dataset_name, "inference_information.json"
        )
        if isfile(inference_information_file):
            zipf.write(
                inference_information_file,
                os.path.relpath(inference_information_file, nnUNet_results),
            )
        inference_information_txt_file = join(
            nnUNet_results, dataset_name, "inference_information.txt"
        )
        if isfile(inference_information_txt_file):
            zipf.write(
                inference_information_txt_file,
                os.path.relpath(inference_information_txt_file, nnUNet_results),
            )
    print("Done")
...
if __name__ == "__main__":
    export_pretrained_model(
        2,
        "/home/fabian/temp/dataset2.zip",
        strict=False,
        export_crossval_predictions=True,
        folds=(0,),
    )
