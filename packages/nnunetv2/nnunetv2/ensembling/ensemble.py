...
def merge_files(
    ...
    segmentation = label_manager.convert_logits_to_segmentation(probabilities).numpy()
    image_reader_writer.write_seg(
        segmentation, output_filename_truncated + output_file_ending, properties
    )
    if save_probabilities:
        np.savez_compressed(
            output_filename_truncated + ".npz", probabilities=probabilities
        )
        save_pickle(probabilities, output_filename_truncated + ".pkl")


def ensemble_folders(
    list_of_input_folders: List[str],
    output_folder: str,
    ...
    files_per_folder = [
        set(subfiles(i, suffix=".npz", join=False)) for i in list_of_input_folders
    ]
    ...
    for f in files_per_folder:
        assert len(s.difference(f)) == 0, (
            "Not all folders contain the same files for ensembling. Please only "
            "provide folders that contain the predictions"
        )
    lists_of_lists_of_files = [
        [join(fl, fi) for fl in list_of_input_folders] for fi in s
    ]
...
def entry_point_ensemble_folders():
    ...
    parser.add_argument(
        "-i", nargs="+", type=str, required=True, help="list of input folders"
    )
...
def ensemble_crossvalidations(
    list_of_trained_model_folders: List[str],
    output_folder: str,
    ...
    for tr in list_of_trained_model_folders:
        ...
        for f in folds:
            if not isdir(join(tr, f"fold_{f}", "validation")):
                raise RuntimeError(
                    f"Expected model output directory does not exist. You must train all requested "
                    f"folds of the speficied model.\nModel: {tr}\nFold: {f}"
                )
            files_here = subfiles(
                join(tr, f"fold_{f}", "validation"), suffix=".npz", join=False
            )
            if len(files_here) == 0:
                raise RuntimeError(
                    f"No .npz files found in folder {join(tr, f'fold_{f}', 'validation')}. Rerun your "
                    f"validation with the --npz flag. Use nnUNetv2_train [...] --val --npz."
                )
            files_per_folder[tr][f] = subfiles(
                join(tr, f"fold_{f}", "validation"), suffix=".npz", join=False
            )
    ...
    for tr, fi in files_per_folder.items():
        ...
        if len(diff) > 0:
            ok = False
            print(
                f"model {tr} does not seem to contain all predictions. Missing: {diff}"
            )
        if not ok:
            raise RuntimeError(
                "There were missing files, see print statements above this one"
            )
    ...
    for tr in list_of_trained_model_folders:
        ...
        for f in folds:
            for fi in files_per_folder[tr][f]:
                ...
                assert fi not in file_mapping[-1].keys(), (
                    f"Duplicate detected. Case {fi} is present in more than "
    ...
    if not overwrite:
        ...
        lists_of_lists_of_files = [
            lists_of_lists_of_files[i] for i in range(len(tmp)) if not tmp[i]
        ]
        output_files_truncated = [
            output_files_truncated[i] for i in range(len(tmp)) if not tmp[i]
        ]
    ...
    shutil.copy(
        join(list_of_trained_model_folders[0], "plans.json"),
        join(output_folder, "plans.json"),
    )
    shutil.copy(
        join(list_of_trained_model_folders[0], "dataset.json"),
