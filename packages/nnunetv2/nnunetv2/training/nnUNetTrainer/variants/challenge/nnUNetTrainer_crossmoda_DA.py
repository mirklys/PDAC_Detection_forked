...
class nnUNetTrainer_crossmoda_DA(nnUNetTrainer):
    def __init__(
        self,
        plans: dict,
        configuration: str,
    ...
    def configure_rotation_dummyDA_mirroring_and_inital_patch_size(self):
        ...
        rotation_for_DA = {
            "x": (-30.0 / 360 * 2.0 * np.pi, 30.0 / 360 * 2.0 * np.pi),  # 256
            "y": (-30.0 / 360 * 2.0 * np.pi, 30.0 / 360 * 2.0 * np.pi),  # 144
            "z": (-30.0 / 360 * 2.0 * np.pi, 30.0 / 360 * 2.0 * np.pi),
        }  # 32

        mirror_axes = (2,)
        initial_patch_size = get_patch_size(
            patch_size[-dim:], *rotation_for_DA.values(), (0.75, 1.35)
        )
    ...
    def get_training_transforms(
        ...
        tr_transforms.append(TumorAug1Transform(p_per_sample=0.2))
        tr_transforms.append(TumorAug2Transform(p_per_sample=0.2))
        tr_transforms.append(CochleaAugTransform(p_per_sample=0.2))
        ...
        tr_transforms.append(ContrastAugmentationTransform(p_per_sample=0.2))
        tr_transforms.append(
            SimulateLowResolutionTransform(
                zoom_range=(0.5, 1),
                per_channel=True,
        ...
        tr_transforms.append(
            GammaTransform((0.7, 1.5), True, True, retain_stats=True, p_per_sample=0.25)
        )
        tr_transforms.append(
            GammaTransform(
                (0.7, 1.5), False, True, retain_stats=True, p_per_sample=0.25
            )
        )
