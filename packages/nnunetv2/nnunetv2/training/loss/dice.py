...
class MemoryEfficientSoftTverskyLoss(nn.Module):
    def __init__(self, alpha=0.5, beta=0.5, apply_nonlin: Callable = None, batch_dice: bool = False, 
        ...
        alpha (float): Penalty for false positives.
        beta (float): Penalty for false negatives.
    ...
    def forward(self, x, y, loss_mask=None):
        ...
        if loss_mask is None:
            ...
            false_neg = (y_onehot * (1 - x)).sum(axes)
            false_pos = ((~y_onehot) * x).sum(axes)
        else:
            intersect = (x * y_onehot * loss_mask).sum(axes)
            false_neg = (y_onehot * (1 - x) * loss_mask).sum(axes)
            false_pos = ((~y_onehot) * x * loss_mask).sum(axes)

        if self.batch_dice:
            if self.ddp:
                intersect = AllGatherGrad.apply(intersect).sum(0)
                false_neg = AllGatherGrad.apply(false_neg).sum(0)
                false_pos = AllGatherGrad.apply(false_pos).sum(0)
                sum_gt = AllGatherGrad.apply(sum_gt).sum(0)

            intersect = intersect.sum(0)
            false_neg = false_neg.sum(0)
            false_pos = false_pos.sum(0)
            sum_gt = sum_gt.sum(0)

        tversky_index = (intersect + self.smooth) / (intersect + self.alpha * false_pos + self.beta * false_neg + self.smooth)
