...
class Convert3DTo2DTransform(AbstractTransform):
    ...
    def __call__(self, **data_dict):
        for k in self.apply_to_keys:
            ...
            assert (
                len(shp) == 5
            ), "This transform only works on 3D data, so expects 5D tensor (b, c, x, y, z) as input."
            data_dict[k] = data_dict[k].reshape(
                (shp[0], shp[1] * shp[2], shp[3], shp[4])
            )
            shape_key = f"orig_shape_{k}"
            assert shape_key not in data_dict.keys(), (
                f"Convert3DTo2DTransform needs to store the original shape. "
...
class Convert2DTo3DTransform(AbstractTransform):
    ...
    def __call__(self, **data_dict):
        for k in self.apply_to_keys:
            shape_key = f"orig_shape_{k}"
            assert shape_key in data_dict.keys(), (
                f"Did not find key {shape_key} in data_dict. Shitty. "
