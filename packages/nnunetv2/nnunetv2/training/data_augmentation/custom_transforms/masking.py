...
class TumorAugTransform(AbstractTransform):
    def __init__(
        self, data_key: str = "data", seg_key: str = "seg", p_per_sample: float = 1
    ):
    ...
    def __call__(self, **data_dict):
        if np.random.uniform() < self.p_per_sample:
            mask = (data_dict[self.seg_key][:, 0] > 0) & (
                data_dict[self.seg_key][:, 0] < 3
            )
    ...
    def __call__(self, **data_dict):
        if np.random.uniform() < self.p_per_sample:
            for i in range(data_dict[self.data_key].shape[0]):
                mask = (data_dict[self.seg_key][i, 0] > 0) & (
                    data_dict[self.seg_key][i, 0] < 3
                )
                global_min = data_dict[self.data_key][i, 0].min() * np.random.uniform(
                    0, 0.5
                )  # a negative value
