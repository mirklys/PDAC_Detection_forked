...
class nnUNetPredictor(object):
    def __init__(
        ...
        if device.type != "cuda":
            print(
                f"perform_everything_on_gpu=True is only supported for cuda devices! Setting this to False"
            )
        ...
        seg_from_prev_stage_files = [
            (
                join(
                    ...
                    i + self.dataset_json["file_ending"],
                )
        ...
        with multiprocessing.get_context("spawn").Pool(
            ...
            for preprocessed in data_iterator:
                ...
                proceed = not check_workers_busy(
                    export_pool, r, allowed_num_queued=2 * len(export_pool._pool)
                )
                while not proceed:
                    print("sleeping")
                    sleep(0.1)
    ...
    def predict_logits_from_preprocessed_data(
        self, data: torch.Tensor, *args, **kwargs
    ) -> torch.Tensor:
        """
    ...
    def _internal_get_sliding_window_slicers(self, image_size: Tuple[int, ...]):
        ...
        if len(self.configuration_manager.patch_size) < len(image_size):
            ...
            steps = compute_steps_for_sliding_window(
                image_size, self.configuration_manager.patch_size, self.tile_step_size
            )
            if self.verbose:
                print(
    ...
    def _internal_maybe_mirror_and_predict(
        ...
        if "code" in kwargs.keys():
            assert (
                len(kwargs["code"]) == x.shape[1]
            ), "The code length should match the input channels.."
            ...
            if "code" in kwargs.keys() and self.trainer_name not in ["nnUNetTrainerMD"]:
                if 0 in mirror_axes:
                    prediction += torch.flip(
                        self.network(torch.flip(x, (2,)), code), (2,)
                    )
                if 1 in mirror_axes:
                    prediction += torch.flip(
                        self.network(torch.flip(x, (3,)), code), (3,)
                    )
                if 2 in mirror_axes:
                    prediction += torch.flip(
                        self.network(torch.flip(x, (4,)), code), (4,)
                    )
                if 0 in mirror_axes and 1 in mirror_axes:
                    prediction += torch.flip(
                        self.network(torch.flip(x, (2, 3)), code), (2, 3)
                    )
                if 0 in mirror_axes and 2 in mirror_axes:
                    prediction += torch.flip(
                        self.network(torch.flip(x, (2, 4)), code), (2, 4)
                    )
                if 1 in mirror_axes and 2 in mirror_axes:
                    prediction += torch.flip(
                        self.network(torch.flip(x, (3, 4)), code), (3, 4)
                    )
                if 0 in mirror_axes and 1 in mirror_axes and 2 in mirror_axes:
                    prediction += torch.flip(
                        self.network(torch.flip(x, (2, 3, 4)), code), (2, 3, 4)
                    )
                ...
                if 0 in mirror_axes and 1 in mirror_axes:
                    prediction += torch.flip(
                        self.network(torch.flip(x, (2, 3))), (2, 3)
                    )
                if 0 in mirror_axes and 2 in mirror_axes:
                    prediction += torch.flip(
                        self.network(torch.flip(x, (2, 4))), (2, 4)
                    )
                if 1 in mirror_axes and 2 in mirror_axes:
                    prediction += torch.flip(
                        self.network(torch.flip(x, (3, 4))), (3, 4)
                    )
                if 0 in mirror_axes and 1 in mirror_axes and 2 in mirror_axes:
                    prediction += torch.flip(
                        self.network(torch.flip(x, (2, 3, 4))), (2, 3, 4)
                    )
                prediction /= num_predictons
        return prediction

    def predict_sliding_window_return_logits(
        self, input_image: torch.Tensor, *args, **kwargs
...
if __name__ == "__main__":
    ...
    img, props = SimpleITKIO().read_images(
        [join(nnUNet_raw, "Dataset003_Liver/imagesTr/liver_63_0000.nii.gz")]
    )
    ret = predictor.predict_single_npy_array(img, props, None, None, False)

    iterator = predictor.get_data_iterator_from_raw_npy_data(
        [img], None, [props], None, 1
    )
