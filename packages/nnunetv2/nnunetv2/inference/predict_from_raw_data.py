...
class nnUNetPredictor(object):
    ...
    def predict_from_files(self,
        ...
        return self.predict_from_data_iterator(data_iterator, save_probabilities, num_processes_segmentation_export, *args, **kwargs)
    ...
    def predict_from_list_of_npy_arrays(self,
        ...
        iterator = self.get_data_iterator_from_raw_npy_data(image_or_list_of_images,
                                                            ...
                                                            truncated_ofname,
                                                            num_processes)
        return self.predict_from_data_iterator(iterator, save_probabilities, num_processes_segmentation_export, *args, **kwargs)
    ...
    def predict_logits_from_preprocessed_data(self, data: torch.Tensor, *args, **kwargs) -> torch.Tensor:
        """
                ...
                try:
                    for params in self.list_of_parameters:
                        ...
                        if prediction is None:
                            prediction = self.predict_sliding_window_return_logits(data, *args, **kwargs)
                        else:
                            prediction += self.predict_sliding_window_return_logits(data, *args, **kwargs)
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor, *args, **kwargs) -> torch.Tensor:
        mirror_axes = self.allowed_mirroring_axes if self.use_mirroring else None

        if 'code' in kwargs.keys():
            assert len(kwargs['code']) == x.shape[1], 'The code length should match the input channels..'
            code = torch.tensor(kwargs['code'])[None, ...].float().cuda()
            x *= code[..., None, None, None]

        if 'code' in kwargs.keys() and self.trainer_name not in ['nnUNetTrainerMD']:
            prediction = self.network(x, code)
        else:
        ...
        if mirror_axes is not None:
            ...
            if 'code' in kwargs.keys() and self.trainer_name not in ['nnUNetTrainerMD']:
                if 0 in mirror_axes:
                    prediction += torch.flip(self.network(torch.flip(x, (2,)), code), (2,))
                if 1 in mirror_axes:
                    prediction += torch.flip(self.network(torch.flip(x, (3,)), code), (3,))
                if 2 in mirror_axes:
                    prediction += torch.flip(self.network(torch.flip(x, (4,)), code), (4,))
                if 0 in mirror_axes and 1 in mirror_axes:
                    prediction += torch.flip(self.network(torch.flip(x, (2, 3)), code), (2, 3))
                if 0 in mirror_axes and 2 in mirror_axes:
                    prediction += torch.flip(self.network(torch.flip(x, (2, 4)), code), (2, 4))
                if 1 in mirror_axes and 2 in mirror_axes:
                    prediction += torch.flip(self.network(torch.flip(x, (3, 4)), code), (3, 4))
                if 0 in mirror_axes and 1 in mirror_axes and 2 in mirror_axes:
                    prediction += torch.flip(self.network(torch.flip(x, (2, 3, 4)), code), (2, 3, 4))
    ...
    def predict_sliding_window_return_logits(self, input_image: torch.Tensor, *args, **kwargs) \
        ...
        with torch.no_grad():
            with torch.autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
                ...
                try:
                    ...
                    if self.use_gaussian:
                        gaussian = compute_gaussian(tuple(self.configuration_manager.patch_size), sigma_scale=1. / 8,
                                                    value_scaling_factor=10,  # was 1000
                                                    device=results_device)
                except RuntimeError:
                ...
                for sl in tqdm(slicers, disable=not self.allow_tqdm):
                    ...
                    prediction = self._internal_maybe_mirror_and_predict(workon, *args, **kwargs)[0].to(results_device)
...
def predict_entry_point():
    ...
    parser.add_argument('-f', nargs='+', type=str, required=False, default=(0, 1, 2, 3, 4),
                        help='Specify the folds of the trained model that should be used for prediction. '
                             'Default: (0, 1, 2, 3, 4)')
    parser.add_argument('-code', nargs='+', type=int, required=False,
                        help='Specify the missing modality condition using a binary code.')
    parser.add_argument('-step_size', type=float, required=False, default=0.5,
                        help='Step size for sliding window prediction. The larger it is the faster but less accurate '
                             'the prediction. Default: 0.5. Cannot be larger than 1. We recommend the default.')
    parser.add_argument('--disable_tta', action='store_true', required=False, default=False,
