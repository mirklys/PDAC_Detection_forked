    ...
    try:
        ...
        for idx in range(len(list_of_lists)):
            ...
            item = {
                ...
                "ofile": (
                    output_filenames_truncated[idx]
...
class PreprocessAdapter(DataLoader):
    ...
    def generate_train_batch(self):
        ...
        data, seg, data_properites = self.preprocessor.run_case(
            files,
            seg_prev_stage,
            self.plans_manager,
            self.configuration_manager,
        ...
        if seg_prev_stage is not None:
            seg_onehot = convert_labelmap_to_one_hot(
                seg[0], self.label_manager.foreground_labels, data.dtype
            )
            data = np.vstack((data, seg_onehot))
...
def preprocess_fromnpy_save_to_queue(
    list_of_images: List[np.ndarray],
    list_of_segs_from_prev_stage: Union[List[np.ndarray], None],
    list_of_image_properties: List[dict],
    truncated_ofnames: Union[List[str], None],
    ...
    try:
        ...
        for idx in range(len(list_of_images)):
            data, seg = preprocessor.run_case_npy(
                list_of_images[idx],
                (
                    list_of_segs_from_prev_stage[idx]
                    if list_of_segs_from_prev_stage is not None
                    else None
                ),
            ...
            if (
                list_of_segs_from_prev_stage is not None
                and list_of_segs_from_prev_stage[idx] is not None
            ):
                seg_onehot = convert_labelmap_to_one_hot(
                    seg[0], label_manager.foreground_labels, data.dtype
                )
            ...
            item = {
                "data": data,
                "data_properites": list_of_image_properties[idx],
                "ofile": (
                    truncated_ofnames[idx] if truncated_ofnames is not None else None
                ),
            }
            success = False
            while not success:
...
def preprocessing_iterator_fromnpy(
    list_of_images: List[np.ndarray],
    list_of_segs_from_prev_stage: Union[List[np.ndarray], None],
    list_of_image_properties: List[dict],
    truncated_ofnames: Union[List[str], None],
    ...
    for i in range(num_processes):
        ...
        queue = manager.Queue(maxsize=1)
        pr = context.Process(
            target=preprocess_fromnpy_save_to_queue,
            args=(
                list_of_images[i::num_processes],
                (
                    list_of_segs_from_prev_stage[i::num_processes]
                    if list_of_segs_from_prev_stage is not None
                    else None
                ),
                ...
                (
                    ...
                    if truncated_ofnames is not None
                    else None
                ),
                plans_manager,
                dataset_json,
