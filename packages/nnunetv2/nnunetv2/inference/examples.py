if __name__ == "__main__":
    ...
    predictor.predict_from_files(
        [
            [join(indir, "liver_152_0000.nii.gz")],
            [join(indir, "liver_142_0000.nii.gz")],
        ],
        [join(outdir, "liver_152.nii.gz"), join(outdir, "liver_142.nii.gz")],
        save_probabilities=False,
        overwrite=True,
        num_processes_preprocessing=2,
        num_processes_segmentation_export=2,
    ...
    img, props = SimpleITKIO().read_images(
        [join(nnUNet_raw, "Dataset003_Liver/imagesTs/liver_147_0000.nii.gz")]
    )
    img2, props2 = SimpleITKIO().read_images(
        [join(nnUNet_raw, "Dataset003_Liver/imagesTs/liver_146_0000.nii.gz")]
    )
    img3, props3 = SimpleITKIO().read_images(
        [join(nnUNet_raw, "Dataset003_Liver/imagesTs/liver_145_0000.nii.gz")]
    )
    img4, props4 = SimpleITKIO().read_images(
        [join(nnUNet_raw, "Dataset003_Liver/imagesTs/liver_144_0000.nii.gz")]
    )
    ...
    ret = predictor.predict_from_data_iterator(
        my_iterator([img, img2, img3, img4], [props, props2, props3, props4]),
        save_probabilities=False,
