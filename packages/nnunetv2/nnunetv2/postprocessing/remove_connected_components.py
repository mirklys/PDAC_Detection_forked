...
def determine_postprocessing(
    ...
    if plans_file_or_dict is None:
        ...
        if not isfile(expected_plans_file):
            raise RuntimeError(
                f"Expected plans file missing: {expected_plans_file}. The plans fils should have been "
    ...
    predicted_files = subfiles(
        folder_predictions, suffix=dataset_json["file_ending"], join=False
    )
    ...
    if not all([i in predicted_files for i in ref_files]):
        print(
            f"WARNING: Not all files in folder_ref were found in folder_predictions. Determining postprocessing "
    ...
    with multiprocessing.get_context("spawn").Pool(num_processes) as pool:
        ...
        do_this = (
            pp_results["foreground_mean"]["Dice"]
            > baseline_results["foreground_mean"]["Dice"]
        )
        if do_this:
            for class_id in pp_results["mean"].keys():
                if (
                    pp_results["mean"][class_id]["Dice"]
                    < baseline_results["mean"][class_id]["Dice"]
                ):
                    do_this = False
                    break
        if do_this:
            print(
                f"Results were improved by removing all but the largest foreground region. "
            ...
            print(
                f"Removing all but the largest foreground region did not improve results!"
            )
        ...
        if len(labels_or_regions) > 1:
            for label_or_region in labels_or_regions:
                ...
                output_here = join(
                    output_folder, "temp", "keep_largest_perClassOrRegion"
                )
                maybe_mkdir_p(output_here)
                ...
                pp_results = load_summary_json(join(output_here, "summary.json"))
                do_this = (
                    pp_results["mean"][label_or_region]["Dice"]
                    > baseline_results["mean"][label_or_region]["Dice"]
                )
                if do_this:
                    print(
                        f"Results were improved by removing all but the largest component for {label_or_region}. "
                    ...
                    if isdir(
                        join(
                            output_folder,
                            "temp",
                            "keep_largest_perClassOrRegion_currentBest",
                        )
                    ):
                        shutil.rmtree(
                            join(
                    ...
                    print(
                        f"Removing all but the largest component for {label_or_region} did not improve results! "
                        f'Dice before: {round(baseline_results["mean"][label_or_region]["Dice"], 5)} '
                        f'after: {round(pp_results["mean"][label_or_region]["Dice"], 5)}'
                    )
    [
        shutil.copy(join(source, i), join(output_folder, i))
        for i in subfiles(source, join=False)
    ]
    ...
    tmp["input_folder"]["mean"] = {
        label_or_region_to_key(k): tmp["input_folder"]["mean"][k]
        for k in tmp["input_folder"]["mean"].keys()
    }
    tmp["postprocessed"]["mean"] = {
        label_or_region_to_key(k): tmp["postprocessed"]["mean"][k]
        for k in tmp["postprocessed"]["mean"].keys()
    }
...
def apply_postprocessing_to_folder(
    ...
    if plans_file_or_dict is None:
        ...
        if not isfile(expected_plans_file):
            raise RuntimeError(
                f"Expected plans file missing: {expected_plans_file}. The plans file should have been "
...
def entry_point_determine_postprocessing_folder():
    parser = argparse.ArgumentParser(
        "Writes postprocessing.pkl and postprocessing.json in input_folder."
    )
...
def entry_point_apply_postprocessing():
    parser = argparse.ArgumentParser(
        "Apples postprocessing specified in pp_pkl_file to input folder."
    )
    parser.add_argument("-i", type=str, required=True, help="Input folder")
    parser.add_argument("-o", type=str, required=True, help="Output folder")
    parser.add_argument(
        "-pp_pkl_file", type=str, required=True, help="postprocessing.pkl file"
    )
    parser.add_argument(
        "-np",
        type=int,
...
if __name__ == "__main__":
    ...
    merged_output_folder = join(
        trained_model_folder, f"crossval_results_folds_{folds_tuple_to_string(folds)}"
    )
    accumulate_cv_results(trained_model_folder, merged_output_folder, folds, 8, False)

    fns, kwargs = determine_postprocessing(
