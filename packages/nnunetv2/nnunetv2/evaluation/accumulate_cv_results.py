...
def accumulate_cv_results(
    ...
    shutil.copy(
        ...
        join(merged_output_folder, "dataset.json"),
    )
    shutil.copy(
        join(trained_model_folder, "plans.json"),
        join(merged_output_folder, "plans.json"),
    )

    did_we_copy_something = False
    for f in folds:
        expected_validation_folder = join(
            trained_model_folder, f"fold_{f}", "validation"
        )
        if not isdir(expected_validation_folder):
            raise RuntimeError(
                f"fold {f} of model {trained_model_folder} is missing. Please train it!"
            )
        predicted_files = subfiles(
            expected_validation_folder, suffix=dataset_json["file_ending"], join=False
        )
        for pf in predicted_files:
            if overwrite and isfile(join(merged_output_folder, pf)):
                raise RuntimeError(
                    f"More than one of your folds has a prediction for case {pf}"
                )
            if overwrite or not isfile(join(merged_output_folder, pf)):
                shutil.copy(
                    join(expected_validation_folder, pf), join(merged_output_folder, pf)
                )
    ...
    if did_we_copy_something or not isfile(join(merged_output_folder, "summary.json")):
        ...
        compute_metrics_on_folder(
            join(nnUNet_raw, plans_manager.dataset_name, "labelsTr"),
