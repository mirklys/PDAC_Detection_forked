...
def save_summary_json(results: dict, output_file: str):
    ...
    results_converted["mean"] = {
        label_or_region_to_key(k): results["mean"][k] for k in results["mean"].keys()
    }
...
def load_summary_json(filename: str):
    ...
    # convert keys in mean metrics
    results["mean"] = {
        key_to_label_or_region(k): results["mean"][k] for k in results["mean"].keys()
    }
    # convert metric_per_case
        ...
        results = pool.starmap(
            ...
            list(
                zip(
                    files_ref,
                    files_pred,
                    [image_reader_writer] * len(files_pred),
...
def evaluate_simple_entry_point():
    ...
    parser.add_argument(
        "-il", type=int, required=False, default=None, help="ignore label"
    )
    parser.add_argument(
        "-o",
        type=str,
