...
def filter_available_models(
    ...
    for trained_model in model_dict:
        ...
        if trained_model["configuration"] not in plans_manager.available_configurations:
            print(
                f"Configuration {trained_model['configuration']} not found in plans {trained_model['plans']}.\n"
        ...
        expected_output_folder = get_output_folder(
            dataset_name_or_id,
        ...
        if not isdir(expected_output_folder):
            raise RuntimeError(
                f"Trained model {trained_model} does not have an output folder. "
...
def generate_inference_command(
    ...
    predict_command = ""
    trained_model_folder = get_output_folder(
        dataset_name_or_id,
        trainer_name,
        plans_identifier,
        configuration_name,
    ...
    if "previous_stage" in plans_manager.available_configurations:
        ...
        predict_command += (
            generate_inference_command(
                ...
                folds,
                None,
                output_folder="OUTPUT_FOLDER_PREV_STAGE",
            )
            + "\n"
        )
    ...
    predict_command = (
        f"nnUNetv2_predict -d {dataset_name_or_id} -i {input_folder} -o {output_folder} -f {fold_str} "
        f"-tr {trainer_name} -c {configuration_name} -p {plans_identifier}"
    )
    if folder_with_segs_from_prev_stage is not None:
        predict_command += (
            f" -prev_stage_predictions {folder_with_segs_from_prev_stage}"
        )
    if save_npz:
        predict_command += " --npz"
...
def find_best_configuration(
    ...
    allowed_trained_models = filter_available_models(
        deepcopy(allowed_trained_models), dataset_name_or_id
    )

    for m in allowed_trained_models:
        output_folder = get_output_folder(
            dataset_name_or_id, m["trainer"], m["plans"], m["configuration"], fold=None
        )
        if not isdir(output_folder) and strict:
            raise RuntimeError(
                f'{dataset_name}: The output folder of plans {m["plans"]} configuration '
                f'{m["configuration"]} is missing. Please train the model (all requested folds!) first!'
            )
        identifier = os.path.basename(output_folder)
        merged_output_folder = join(
            output_folder, f"crossval_results_folds_{folds_tuple_to_string(folds)}"
        )
        accumulate_cv_results(
            output_folder, merged_output_folder, folds, num_processes, overwrite
        )
        all_results[identifier] = {
            "source": merged_output_folder,
            "result": load_summary_json(join(merged_output_folder, "summary.json"))[
                "foreground_mean"
            ]["Dice"],
        }
    ...
    if allow_ensembling:
        for i in range(len(allowed_trained_models)):
            for j in range(i + 1, len(allowed_trained_models)):
                ...
                output_folder_1 = get_output_folder(
                    dataset_name_or_id,
                    m1["trainer"],
                    m1["plans"],
                    m1["configuration"],
                ...
                output_folder_2 = get_output_folder(
                    dataset_name_or_id,
                    m2["trainer"],
                    m2["plans"],
                    m2["configuration"],
                ...
                output_folder_ensemble = join(
                    nnUNet_results, dataset_name, "ensembles", identifier
                )

                ensemble_crossvalidations(
                    [output_folder_1, output_folder_2],
    ...
    best_keys = [
        k for k in all_results.keys() if all_results[k]["result"] == best_score
    ]  # may never happen but theoretically
    ...
    return_dict = {
        ...
        "best_model_or_ensemble": {
            ...
            "result_on_crossval_post_pp": load_json(
                join(all_results[best_key]["source"], "postprocessed", "summary.json")
            )["foreground_mean"]["Dice"],
            "postprocessing_file": join(
                all_results[best_key]["source"], "postprocessing.pkl"
            ),
    ...
    save_json(
        return_dict, join(nnUNet_results, dataset_name, "inference_information.json")
    )  # save this so that we don't have to run this
    ...
    print_inference_instructions(
        return_dict,
        instructions_file=join(
            nnUNet_results, dataset_name, "inference_instructions.txt"
        ),
    )
    return return_dict
...
def print_inference_instructions(
    ...
    if (
        len(inference_info_dict["best_model_or_ensemble"]["selected_model_or_models"])
        > 1
    ):
        is_ensemble = True
    ...
    for j, i in enumerate(
        inference_info_dict["best_model_or_ensemble"]["selected_model_or_models"]
    ):
        ...
        _print_and_maybe_write_to_file(
            generate_inference_command(
                ...
                pl,
                tr,
                inference_info_dict["folds"],
                save_npz=is_ensemble,
    ...
    if is_ensemble:
        ...
        _print_and_maybe_write_to_file(
            f"nnUNetv2_ensemble -i {output_folder_str} -o {output_ensemble} -np {default_num_processes}"
        )
...
def dumb_trainer_config_plans_to_trained_models_dict(
    trainers: List[str], configs: List[str], plans: List[str]
):
    """
...
def accumulate_crossval_results_entry_point():
    parser = argparse.ArgumentParser(
        "Copies all predicted segmentations from the individual folds into one joint "
        "folder and evaluates them"
    )
    parser.add_argument("dataset_name_or_id", type=str, help="Dataset Name or id")
    parser.add_argument(
        "-c", type=str, required=True, default="3d_fullres", help="Configuration"
    )
    parser.add_argument(
        "-o",
        type=str,
    ...
    args = parser.parse_args()
    trained_model_folder = get_output_folder(
        args.dataset_name_or_id, args.tr, args.p, args.c
    )

    if args.o is None:
        merged_output_folder = join(
            trained_model_folder,
            f"crossval_results_folds_{folds_tuple_to_string(args.f)}",
        )
    else:
...
if __name__ == "__main__":
    find_best_configuration(4, default_trained_models, True, 8, False, (0, 1, 2, 3, 4))
