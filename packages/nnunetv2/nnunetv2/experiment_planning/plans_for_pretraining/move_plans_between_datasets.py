...
def move_plans_between_datasets(
    ...
    assert isdir(source_folder), (
        f"Cannot move plans because preprocessed directory of source dataset is missing. "
    ...
    assert isfile(source_plans_file), (
        f"Source plans are missing. Run the corresponding experiment planning first! "
    ...
    if target_plans_identifier != source_plans_identifier:
        for c in source_plans["configurations"].keys():
            ...
            if old_identifier.startswith(source_plans_identifier):
                new_identifier = (
                    target_plans_identifier
                    + old_identifier[len(source_plans_identifier) :]
                )
            else:
    ...
    rw = determine_reader_writer_from_dataset_json(
        target_dataset_json, some_file, allow_nonmatching_filename=True, verbose=False
    )
    ...
    save_json(
        source_plans,
        join(
            nnUNet_preprocessed, target_dataset_name, target_plans_identifier + ".json"
        ),
        sort_keys=False,
    )
