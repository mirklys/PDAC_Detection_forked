...
class ExperimentPlanner(object):
    ...
    def determine_normalization_scheme_and_whether_mask_is_used_for_norm(
        ...
        if "channel_names" not in self.dataset_json.keys():
            print(
                'WARNING: "modalities" should be renamed to "channel_names" in dataset.json. This will be '
                "enforced soon!"
            )
        modalities = (
    ...
    def get_plans_for_configuration(
        ...
        while estimate > reference:
            ...
            axis_to_be_reduced = np.argsort(patch_size / median_shape[: len(spacing)])[
                -1
            ]
        ...
        plan = {
            ...
            "unet_max_num_features": (
                ...
                if len(spacing) == 3
                else self.UNet_max_features_2d
            ),
        ...
        if new_median_shape_transposed[0] != 1:
            ...
            while (
                ...
                plan_3d_lowres = self.get_plans_for_configuration(
                    ...
                    float(
                        np.prod(median_num_voxels) * self.dataset_json["numTraining"]
                    ),
                )
    ...
    def save_plans(self, plans):
        ...
        plans_file = join(
            nnUNet_preprocessed, self.dataset_name, self.plans_identifier + ".json"
        )
