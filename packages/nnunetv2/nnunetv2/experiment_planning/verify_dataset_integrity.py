...
def verify_labels(
    label_file: str, readerclass: Type[BaseReaderWriter], expected_labels: List[int]
) -> bool:
    rw = readerclass()
    ...
    if len(found_labels) == 0 and found_labels[0] == 0:
        print(
            "WARNING: File %s only has label 0 (which should be background). This may be intentional or not, "
            "up to you." % label_file
        )
    if len(unexpected_labels) > 0:
        print(
            "Error: Unexpected labels found in file %s.\nExpected: %s\nFound: %s"
            % (label_file, expected_labels, found_labels)
        )
        return False
    return True
...
def check_cases(
    ...
    readerclass: Type[BaseReaderWriter],
    file_ending: str,
) -> bool:
    rw = readerclass()
    ret = True
    ...
    pattern = re.compile(
        re.escape(case_identifier) + r"_\d\d\d\d" + re.escape(file_ending)
    )
    files_image = [
        join(base_folder, "imagesTr", i)
        for i in subfiles(
            join(base_folder, "imagesTr"),
    ...
    if np.any(np.isnan(images)):
        print(
            f"Images of case identifier {case_identifier} contain NaN pixel values. You need to fix that by "
            f"replacing NaN values with something that makes sense for your images!"
        )
        ret = False
    if np.any(np.isnan(segmentation)):
        print(
            f"Segmentation of case identifier {case_identifier} contains NaN pixel values. You need to fix that."
        )
        ret = False
...
def verify_dataset_integrity(folder: str, num_processes: int = 8) -> None:
    ...
    training_identifiers = get_identifiers_from_splitted_dataset_folder(
        join(folder, "imagesTr"), file_ending=file_ending
    )

    # check if the right number of training cases is present
    assert len(training_identifiers) == expected_num_training, (
        "Did not find the expected number of training cases "
        "(%d). Found %d instead.\nExamples: %s"
        % (expected_num_training, len(training_identifiers), training_identifiers[:5])
    )
    ...
    assert all(labels_present), (
        "not all training cases have a label file in labelsTr. Fix that. Missing: %s"
        % missing
    )
    ...
    reader_writer_class = determine_reader_writer_from_dataset_json(
        dataset_json,
        join(folder, "imagesTr", training_identifiers[0] + "_0000" + file_ending),
    )
    ...
    with multiprocessing.get_context("spawn").Pool(num_processes) as p:
        result = p.starmap(
            verify_labels,
            zip(
                [join(folder, "labelsTr", i) for i in labelfiles],
                [reader_writer_class] * len(labelfiles),
        ...
        result = p.starmap(
            check_cases,
            zip(
                [folder] * expected_num_training,
                training_identifiers,
                [num_modalities] * expected_num_training,
                [reader_writer_class] * expected_num_training,
                [file_ending] * expected_num_training,
            ),
        )
        if not all(result):
            raise RuntimeError(
