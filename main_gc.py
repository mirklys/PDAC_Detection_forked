...
def predict(
    ...
    output_dir,
    task: int = "107",
    trainer: str = "nnUNetTrainerTverskyLoss",
...
def run(args):
    ...
    for npz_fp, img_fp in zip(npz_fps, img_fps):
        ...
        detection_map, pat_pred = GetFullSizDetectionMap(
            prediction_postprocessed, crop_coordinates[case_id], itk_img, args.inv_alpha
        )
    ...
    write_json_file(
        location=Path(args.output_dir) / "pdac-likelihood.json", content=payload
    )

    print(f"\nInference time: {(time.time()-start)/len(npz_fps):.2f} seconds per image")
    print(
        f"Patient-level scores are saved at: {osp.join(args.output_dir, 'pdac-likelihood.json')}"
    )
